...
@(test)
reference_variables_in_function_with_empty_line_at_top_of_file :: proc(
	t: ^testing.T,
) {
	source := test.Source {
		main = `
...
@(test)
ast_reference_variable_uses_from_declaration_with_selector_expr :: proc(
	t: ^testing.T,
) {
	source := test.Source {
		main     = `package test
...
@(test)
ast_reference_variable_declaration_field_with_selector_expr :: proc(
	t: ^testing.T,
) {
	source := test.Source {
		main     = `package test
...
ast_reference_cast_proc_param_with_param_expr :: proc(t: ^testing.T) {
	...
	locations := []common.Location {
		...
		{
			range = {
				start = {line = 14, character = 19},
				end = {line = 14, character = 22},
			},
		},
		{
			range = {
				start = {line = 14, character = 43},
...
@(test)
ast_references_nested_using_bit_field_field_from_declaration :: proc(
	t: ^testing.T,
) {
	source := test.Source {
		main = `package test
